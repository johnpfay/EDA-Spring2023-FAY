...
Plot1 <- PeterPaul.chem.nutrients %>% 
  filter(depth==0) %>% 
  ggplot(aes(x=year4,y=temperature_C)) +
...
Plot2 <- PeterPaul.chem.nutrients %>% 
  ggplot(aes(x=sampledate,y=temperature_C)) +
...
EPAair %>% 
  ggplot(aes(x=PM2.5, y=Ozone,color=meanLat)) + 
  geom_point(alpha=0.5) + 
  geom_smooth(method='lm') + 
  scale_color_viridis(option ='viridis',alpha = 0.5) +
  labs(color='Latitude')
