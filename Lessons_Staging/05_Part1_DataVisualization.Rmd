...
`ggplot` has only a few key rules: 

 * The first line of ggplot code always starts with `ggplot()`. Think of this as creating the canvas for your plot.
   * A data frame containing the data to be plotted is typically specified within the `ggplot()` function.
   * Additional datasets can be specified in subsequent layers or "geoms".
   * Aesthetics, including which columns are included in the plot, can also be specified in the `ggplot()` function
   
 * Following the `ggplot()` function, one or more layers, or `geoms` are added to your plot. These specify the type of plot that will be added to your plotting canvas (e.g., histogram, bar, scatter).
   * Additional aesthetics can be provided with each geom that are specific to how that particular layer will display the data.
   * You can also specify a different dataframe with each geom to be the source of the data to be plotted 
   
 * There are other elements that you can add to your ggplot that we will discuss later. 
...
# Scatterplot
ggplot() +
  geom_point(data=EPAair,aes(x = Date, y = Ozone))
